...
- [X] T001 Install html2canvas dependency: `npm install html2canvas --save`
- [X] T002 Install html2canvas types: `npm install @types/html2canvas --save-dev`
- [X] T003 [P] Create shared TypeScript types in src/shared/types/export.ts
- [X] T004 [P] Create shared TypeScript types in src/shared/types/clipboard.ts
- [X] T005 [P] Create ExportLogger service in src/main/services/export/ExportLogger.ts
- [X] T006 Create electron-store integration for export settings in src/main/services/export/ExportSettingsStore.ts
...
- [X] T007 [P] Create ClipboardService interface in src/renderer/services/ClipboardService.ts
- [X] T008 [P] Create DiagramCaptureService in src/renderer/services/DiagramCaptureService.ts
- [X] T009 [P] Create base ErrorDialog component in src/renderer/components/ErrorDialog.tsx
- [X] T010 Create exportStore with Zustand in src/renderer/stores/exportStore.ts
...
- [X] T011 [US1] Create PdfExportService in src/main/services/export/PdfExportService.ts
- [X] T012 [US1] Implement printToPDF wrapper with options handling in PdfExportService
- [X] T013 [US1] Add progress event emission in PdfExportService
- [X] T014 [US1] Create IPC handler for 'export:pdf:single' in src/main/services/ipc/exportHandlers.ts
- [X] T015 [US1] Create preload API bridge in src/preload/exportApi.ts
- [X] T016 [US1] Register export IPC handlers in main process entry (src/main/index.ts)
- [X] T017 [P] [US1] Create ExportProgressDialog component in src/renderer/components/ExportProgressDialog.tsx
- [X] T018 [P] [US1] Create useExport hook in src/renderer/hooks/useExport.ts
- [X] T019 [US1] Add "Export to PDF" menu item to application menu
- [X] T020 [US1] Wire export menu action to useExport hook with file save dialog
- [X] T021 [US1] Add print CSS styles for page breaks in renderer styles
- [X] T022 [US1] Handle export errors with ErrorDialog and retry button
...
- [X] T023 [US2] Implement PNG capture method in DiagramCaptureService using html2canvas
- [X] T024 [US2] Implement SVG extraction method in DiagramCaptureService
- [X] T025 [US2] Implement mermaid code extraction in DiagramCaptureService
- [X] T026 [US2] Implement copyImage method in ClipboardService
- [X] T027 [US2] Implement copyText method in ClipboardService for code
- [X] T028 [P] [US2] Create DiagramHoverButtons component in src/renderer/components/DiagramHoverButtons.tsx
- [X] T029 [P] [US2] Implement hover timing logic (200ms show delay, 500ms hide delay) in DiagramHoverButtons
- [X] T030 [P] [US2] Create useDiagramActions hook in src/renderer/hooks/useDiagramActions.ts
- [X] T031 [US2] Add CSS styles for hover button positioning and animations
- [X] T032 [US2] Integrate DiagramHoverButtons into markdown renderer for mermaid blocks
- [X] T033 [US2] Implement "Copy as PNG" button action
- [X] T034 [US2] Implement "Copy as SVG" button action
- [X] T035 [US2] Implement "Copy Code" button action
- [X] T036 [US2] Implement "Download" button action with save dialog
- [X] T037 [US2] Add success toast notifications for copy operations
- [X] T038 [US2] Add error handling for clipboard permission denied
...
- [X] T039 [P] [US6] Create DiagramTabView component in src/renderer/components/DiagramTabView.tsx
- [X] T040 [P] [US6] Add zoom controls to DiagramTabView
- [X] T041 [P] [US6] Add pan controls to DiagramTabView
- [X] T042 [US6] Implement "Open in New Tab" button action in DiagramHoverButtons
- [X] T043 [US6] Add tab management to application state (open, close, switch)
- [X] T044 [US6] Style DiagramTabView for full-size display
...
- [X] T045 [US3] Implement copyText method with multi-format support in ClipboardService
- [X] T046 [US3] Create TextSelectionService in src/renderer/services/TextSelectionService.ts
- [X] T047 [US3] Implement plain text extraction from selection
- [X] T048 [US3] Implement markdown extraction from selection (preserve syntax)
- [X] T049 [US3] Implement rich text/HTML extraction from selection
- [X] T050 [P] [US3] Create CopyFormatPicker component in src/renderer/components/CopyFormatPicker.tsx
- [X] T051 [P] [US3] Create useCopyShortcuts hook in src/renderer/hooks/useCopyShortcuts.ts
- [X] T052 [US3] Implement Ctrl+C handler (copy as rich text default)
- [X] T053 [US3] Implement Ctrl+Shift+C handler (show format picker)
- [X] T054 [US3] Create context menu for text selection with format options
- [X] T055 [US3] Integrate useCopyShortcuts hook into document view
- [X] T056 [US3] Add format picker modal styling and positioning
- [X] T057 [US3] Test rich text paste in Microsoft Word
- [X] T058 [US3] Test rich text paste in Microsoft Teams
- [X] T059 [US3] Test markdown paste in VSCode
- [X] T060 [US3] Add clipboard error handling with user-friendly messages
...
- [X] T061 [US4] Create FolderExportService in src/main/services/export/FolderExportService.ts
- [X] T062 [US4] Implement markdown file collection (recursive folder traversal)
- [X] T063 [US4] Implement file ordering by hierarchy
- [X] T064 [US4] Implement cover page HTML generation
- [X] T065 [US4] Implement table of contents generation with links
- [X] T066 [US4] Implement combined HTML document builder
- [X] T067 [US4] Add page break CSS between documents
- [X] T068 [US4] Add IPC handler for 'export:pdf:folder' in exportHandlers.ts
- [X] T069 [US4] Update preload API with exportFolderToPdf method
- [X] T070 [US4] Update exportStore with folder export state
- [X] T071 [US4] Add progress reporting per-file during folder export
- [X] T072 [US4] Add "Export Folder to PDF" to folder context menu
- [X] T073 [US4] Implement folder export dialog with options (include subfolders, custom title)
- [X] T074 [US4] Handle non-markdown files (skip with optional logging)
- [X] T075 [US4] Add file count limit enforcement (max 50 documents)
- [X] T076 [US4] Test with folder containing 10+ documents
...
- [X] T077 [US5] Add "Export to PDF" to file context menu in tree view
- [X] T078 [US5] Implement background file rendering (load in hidden window)
- [X] T079 [US5] Wire context menu action to export service
- [X] T080 [US5] Show export progress notification (non-blocking)
- [X] T081 [US5] Handle export completion notification
- [X] T082 [US5] Handle export errors with error notification
...
- [X] T083 [P] Implement export settings persistence (default page size, margins, output directory)
- [X] T084 [P] Create settings UI for export preferences
- [X] T085 [P] Implement export logs viewer with "Open Logs" button in error dialogs
- [X] T086 [P] Add IPC handlers for settings get/update (export:settings:get, export:settings:update)
- [X] T087 [P] Add IPC handlers for logs (export:logs:get, export:logs:open-folder)
- [X] T088 [P] Implement recent exports tracking (store last 10 exports)
- [X] T089 Add keyboard shortcut documentation to help/about dialog
- [X] T090 Add export feature to application help documentation
- [X] T091 Performance optimization: Cache rendered HTML for repeated exports
- [X] T092 Performance optimization: Limit concurrent export jobs (max 3)
- [X] T093 Accessibility: Add aria-labels to all export buttons and dialogs
- [X] T094 Accessibility: Ensure keyboard navigation for all export features
- [X] T095 Add memory monitoring for large document exports
- [X] T096 Implement export cancellation (cancel button in progress dialog)
- [X] T097 Add telemetry for export feature usage (anonymous, opt-in)
- [X] T098 Final manual testing across all user stories
- [X] T099 Update CHANGELOG.md with export features
- [X] T100 Update README.md with export feature documentation
