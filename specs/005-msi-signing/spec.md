...
- What happens when the certificate expires during the middle of a release cycle?
- How does the system handle certificate password changes or rotation in GitHub Secrets?
- What happens if the GitHub Actions workflow fails during the signing step?
- How does the user verify that an MSI was actually signed (and not just built without signing)?
- What happens if someone tries to install the MSI on a machine without trusting the certificate first?
- How does the system handle multiple certificates (e.g., test vs. production)?
...
- **FR-002**: The certificate and private key MUST be stored securely in GitHub Secrets
...
- **FR-008**: The system MUST provide documentation on how to import the certificate on other machines to avoid "Unknown publisher" warnings
- **FR-009**: The GitHub Actions workflow MUST fail if signing fails, preventing unsigned installers from being released
...
- **Certificate Password**: A secret passphrase protecting the private key in the certificate file
